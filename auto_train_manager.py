...
class TrainingManager:
    def __init__(
        ...
        start_script="cmds/train_rl_mixed.sh",
        log_file=None,
    ):
        ...
        self.loss_pattern_little = re.compile(
            r"\[.*rl_adapter_little.*\] Step: \d+, Loss: ([\d\.]+)"
        )
        self.reward_pattern_main = re.compile(
            r"\[.*rl_adapter_main.*\] Step: \d+, .*Reward: ([\d\.]+)"
        )
        self.reward_pattern_little = re.compile(
            r"\[.*rl_adapter_little.*\] Step: \d+, .*Reward: ([\d\.]+)"
        )
    ...
    def prepare_checkpoints(self):
        ...
        if len(existing_new) == len(targets):
            print(
                f"[{datetime.now()}] 检测到现有的双 Agent 检查点: {', '.join(existing_new)}。将直接加载继续训练。"
            )
            return
        ...
        if old_pth.exists():
            ...
            if existing_new:
                print(
                    f"[{datetime.now()}] 部分双 Agent 检查点已存在: {', '.join(existing_new)}。缺失的部分将重新开始。"
                )
            else:
                print(
                    f"[{datetime.now()}] 未发现任何历史检查点 (old or new)。Agent 将从头开始训练。"
                )
                ...
                try:
                    ...
                    if new_data:
                        ...
                        if (
                            ...
                            if reward_main:
                                print(
                                    f"   [Progress] Step: {len(self.reward_history_main)} | Main Reward: {self.reward_history_main[-1]:.4f} | Little Reward: {self.reward_history_little[-1] if self.reward_history_little else 0:.4f}"
                                )
                            elif tps_matches:
                                print(
...
if __name__ == "__main__":
    ...
    manager = TrainingManager(
        model_series_name=args.model, start_script="cmds/train_rl_mixed.sh"
    )
    manager.run_manager()
